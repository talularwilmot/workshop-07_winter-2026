...
```{r abalone-model}
abalone_model <- lm(
  change_in_area ~ mean_ph, # change in shell surface area per data as a function of pH
  data = abalone_clean      # dataL clean abalone data set
)
```
...
**Personal notes:**Residuals seem homoscedastic (consistent across values) based on Residuals vs Fitted and Scale-Location plots. Residuals also seem normally distributed based on QQplot. No major outliers based on Residuals vs. Leverage plot. 

**Written communiation:** We evaluated model assumptions visually. For homoscedacity of residuals we used Residuals vs Fitted and Scale0Location plots. For normality of residuals, we used QQ plots. Based on our diagnostics, there were no major deviations from homoscedasticity or normality of residuals. There were also no outliers that might influence model estimates or predictions. 
...
**The slope estiamte is 2.98 +/- 0.76 (SE). With each increase in pH, the model predicts an increase of 2.98 +/- 0.76 (SE) in change in shell surface area per day (mm^2^ d^-1^)**  

What is the intercept estimate (with standard error?)  

**-18.5 +/- 6.16 (SE). When pH = 0, the models predicts that abalone change in shell surface area per day is -18.5 +/- 6.16. (Note to self: this is not necessarily biologically realistic, but is is something the model generates as an estimate.**
